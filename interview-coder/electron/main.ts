...
const state = {
  ...
  PROCESSING_EVENTS: {
    ...
    SCREENSHOT_BASE64S: "screenshot-base64s",
  } as const,
};
...
function showMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    state.mainWindow.setOpacity(state.opacity);
    state.isWindowVisible = true;
  }
}
...
function increaseOpacity(): void {
  if (!state.mainWindow) return;
  state.opacity = Math.min(1.0, state.opacity + 0.1);
  state.mainWindow.setOpacity(state.opacity);
}

function decreaseOpacity(): void {
  if (!state.mainWindow) return;
  state.opacity = Math.max(0.2, state.opacity - 0.1);
  state.mainWindow.setOpacity(state.opacity);
}
